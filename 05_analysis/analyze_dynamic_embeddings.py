#!/usr/bin/env python
"""
Runs a suite of analyses on the ALOFT dataset's SBERT embeddings.

This script is hardcoded for a specific, reproducible analysis pipeline. It runs
a predefined list of comparisons using dynamic (SBERT) embeddings, generating
...
To run, simply execute from the terminal:
    python 05_analysis/analyze_dynamic_embeddings.py
...
def run_classification(X: np.ndarray, y: np.ndarray, class_names: list[str], all_texts: list[str] | None) -> str:
    ...
    # --- Generate reports ---
    report_str = "I. EMBEDDING SPACE SEPARABILITY ANALYSIS\n"
    report_str += "----------------------------------------\n\n"
    ...
    if all_texts:
        report_str += "\n\nIV. QUALITATIVE ANALYSIS: NOTEWORTHY EXAMPLES\n"
        report_str += "---------------------------------------------\n\n"
...
def analyze_distances(
    ...
    report = "\n\nII. SEMANTIC DISTANCE & OUTLIER ANALYSIS\n"
    report += "------------------------------------------\n\n"
    report += "Hypothesis:\n"
    ...
    for (i, j) in itertools.combinations(range(len(vectors_list)), 2):
        ...
        plot_path = out_dir / f"distance_dist{analysis_suffix}_{label1.replace(' ', '_')}_vs_{label2.replace(' ', '_')}.png"
...
def run_structural_analysis(X: np.ndarray, y: np.ndarray, vectors_list: list[np.ndarray], labels_list: list[str]) -> str:
    ...
    report = "\n\nIII. GLOBAL & STRUCTURAL ANALYSIS\n"
    report += "---------------------------------\n\n"
    report += "Hypothesis:\n"
...
def main():
    ...
    # Each item is a list of paths for a single comparison using SBERT embeddings.
    analyses_to_run = [
        # 1. Within-Book (Goodreads)
        [
            sbert_dir / "goodreads_sample_quote.npz",
            sbert_dir / "google_books_length_matched_snippet.npz",
        ],
        # 2. Within-Book (T50)
        [
            sbert_dir / "t50_quote.npz",
            sbert_dir / "t50_quote-free_context_length_matched.npz",
        ],
        # 3. Literary vs. Informational (Sample)
        [
            sbert_dir / "goodreads_sample_quote.npz",
            sbert_dir / "non-literary_baseline.npz",
        ],
        # 4. Literary vs. Informational (Popular) - NEW
        [
            sbert_dir / "goodreads_popular_quote.npz",
            sbert_dir / "non-literary_baseline.npz",
        ],
    ...
    pooled_extraordinary_stems = ["goodreads_popular_quote", "goodreads_sample_quote", "t50_quote"]
    pooled_ordinary_stems = ["google_books_length_matched_snippet", "t50_quote-free_context_length_matched", "non-literary_baseline"]
    ...
    for i, analysis_config in enumerate(all_analysis_configs):
        print("-" * 80)
        print(f"Starting Analysis {i+1}/{len(all_analysis_configs)}...")
        
        is_pooled = analysis_config is None
        
        if is_pooled:
            extraordinary_paths = [sbert_dir / f"{stem}.npz" for stem in pooled_extraordinary_stems]
            ordinary_paths = [sbert_dir / f"{stem}.npz" for stem in pooled_ordinary_stems]
        ...
        if is_pooled:
            ...
            for group_idx, group_paths in enumerate([extraordinary_paths, ordinary_paths]):
                ...
                for path in group_paths:
                    ...
                    column_name = path.stem.replace("_", " ").replace("glove-", "").replace("t50-", "t50 ").title()
                    if df is not None and column_name in df.columns:
                        texts_for_class = df.loc[ids, column_name].astype(str).tolist()
            ...
            for j, path in enumerate(embedding_paths):
                ...
                labels_by_class.append(column_name)
                all_vectors.append(vectors)
                all_labels_text.extend([column_name] * len(vectors))
